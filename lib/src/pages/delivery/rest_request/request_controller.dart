...
class RestRequestController {
  ...
  Future init(BuildContext context, Function refresh) async {
    ...
    socket?.on('positionAD/', (data) {
      data['id'] != 'r1'
          ? addMarker(data['id'], data['lat'], data['lng'], 'Repartidor', '',
              deliveryMarker!)
  ...
  void emitPosition() {
    socket?.emit('positionAD',
        {'lat': _position?.latitude, 'lng': _position?.longitude, 'id': 'r'});
  }
...
//setPolylines(from, to);
      ...
      _positionStream = Geolocator.getPositionStream(
        ...
        //   addMarker(user.name?? "", _position!.latitude, _position!.longitude,
        //       'Tu posicion', '', deliveryMarker!);
